...
# ==============================================================================
# Add start_idx and end_idx to the function definition
def extract_dmrs_evidence(
    input_json_path: str, output_json_path: str, start_idx: int, end_idx: int
):
    ...
    # SLICE THE DATASET INTO THE SPECIFIC CHUNK
...
if __name__ == "__main__":
    # Ensure two numbers are provided in the terminal
    if len(sys.argv) < 3:
        print("Usage: python script.py <start_index> <end_index>")
        print("Example: python script.py 0 50")
        sys.exit(1)

    start = int(sys.argv[1])
    end = int(sys.argv[2])

    # Automatically name the file so they don't overwrite each other!
    out_file = f"extracted_evidence_{start}_to_{end}.json"

    print(f"Starting job for items {start} through {end}...")
    extract_dmrs_evidence("input_data/test.json", out_file, start, end)
